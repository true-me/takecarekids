...
#define kPRBGColor [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0]
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self setHeaderEnable:NO];
        [self setFooterEnable:NO];
        
        [_headerView updateRefreshDate:[NSDate date]];
        [_footerView updateRefreshDate:[NSDate date]];
...
- (void)setFooterEnable:(BOOL)footerEnable
{
    _footerEnable = footerEnable;
    _footerView.hidden = !_footerEnable;
}

- (void)setHeaderEnable:(BOOL)headerEnable{
    _headerEnable = headerEnable;
    _headerView.hidden = !_headerEnable;
}
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offset.y < -kPROffsetY) {   //header totally appeard
        ...
        if (_footerView.state != kPRStateHitTheEnd && offset.y > 0) {
