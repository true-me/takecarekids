...
- (void)dealloc
{
    [leftViews release];
    [rightViews release];
    [contentsRight release];     
    [contentsLeft release];
    [font release];
    [rightFont release];
    [solidUnderline release];
    [super dealloc];
}
...
- (void)setUnderlineType:(UnderlineType)type {
    ...
    default:
        NSLog(@"%@, %d", self.solidUnderline, [self.solidUnderline retainCount]);
...
- (CALayer *)solidUnderline {
    ...
    solidUnderline = [[CALayer alloc] init];
