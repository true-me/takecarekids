...
{
    CGFloat height = [DesireCell getJSHeight:text jsViewWith:jsView.frame.size.width];
...
{
//    self.contentTF.text = group.groupName;
////    self.JSContentTF.text = status.text;
//    self.userNameLB.text = group.imageName;
    ...
    POHorizontalList *list = [[POHorizontalList alloc] initWithFrame:CGRectMake(0.0, 0.0, 320.0, 155.0) title:@"" items:itemsPerRow];
    [list setDelegate: theDelegate];
...
//    CGSize size = [[NSString stringWithFormat:@"%d",status.retweetsCount] sizeWithFont:[UIFont fontWithName:@"STHeitiSC-Light" size:10]];
...
//    size = [[NSString stringWithFormat:@"%d     :%d",status.commentsCount,status.retweetsCount] sizeWithFont:[UIFont fontWithName:@"STHeitiSC-Light" size:10]];
...
// 取消时间戳的位置调整
    //[timeLB sizeToFit];
    frame = timeLB.frame;
    CGSize size = [timeLB.text sizeWithFont:timeLB.font];
...
//    }
    ...
    if (bgImage.image == nil)
    {
        bgImage.image = [[UIImage imageNamed:@"singledesire-bg.png"] stretchableImageWithLeftCapWidth:252 topCapHeight:110];
    }
    
    if (desireBgImage.image == nil)
    {
        desireBgImage.image = [[UIImage imageNamed:@"bubbleself.png"] stretchableImageWithLeftCapWidth:15 topCapHeight:13];
    }
